...
form.addEventListener("submit", function (event) {
  ...
  if (type === 'one' &&selectedCampuses.length === 0) {
    errored = true;
    amtErrored++;
    output.textContent += `${amtErrored} - Please select at least one campus.\n`;
  }
  ...
  if (type ==='many' && note === ''){
    errored = true;
    amtErrored++;
    output.textContent += `${amtErrored} - Please add a note explaining your travel\n preferences.\n`;
  ...
  if (type === "many" && selectedCampuses.length < 2){
    errored = true;
    amtErrored++;
    output.textContent += `${amtErrored} - Please select at least two campuses if you choose 'Many'.\n`;
